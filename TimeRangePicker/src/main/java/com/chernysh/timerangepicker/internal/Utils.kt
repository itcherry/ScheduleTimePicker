...
internal fun getAngleFromDecart(
  circleCenterPoint: PointF,
  x: Float,
  y: Float
): Float {
  val degree = atan((y - circleCenterPoint.y) / (x - circleCenterPoint.x)).radiansToDegrees() + 90
...
internal fun Context.getDecartCoordinates(
  timeTextWith: Float,
  radius: Float,
  angle: Float
): PointF =
...
internal fun RectF.containsClick(
  context: Context,
  x: Float,
  y: Float
): Boolean {
